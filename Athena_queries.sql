...
CREATE EXTERNAL TABLE IF NOT EXISTS `ptb2demo`.`data` (
  `id` string,
  `name` string,
  `username` string,
  `tweet` string,
...
LOCATION 's3://ptb2-olusegun2/demo/data.csv/'
TBLPROPERTIES ('classification' = 'csv');


--  2. create the prediction table
CREATE EXTERNAL TABLE IF NOT EXISTS `ptb2demo`.`prediction` (
  `label` int,
  `tweet` string,
  `tokens` array < string >,
...
LOCATION 's3://ptb2-olusegun2/demo/predictions.parquet/'
TBLPROPERTIES ('classification' = 'parquet');


-- 3. create the words table
CREATE TABLE "ptb2demo"."words" WITH (format = 'parquet') AS
select tweet,
  words
from prediction
...
CREATE TABLE "ptb2demo"."clean_location" WITH (format = 'TEXTFILE', field_delimiter = ',') AS with a as (
  SELECT *,
    regexp_replace(
      location,
      '["]+',
...
CREATE TABLE "ptb2demo"."clean_data" WITH (format = 'TEXTFILE', field_delimiter = ',') AS with a as (
  SELECT *,
    regexp_replace(
      location,
      '["]+',
